...
- **Dashboard**: `dashboard.mc.danny.is`
...
```
Browser visits https://dashboard.mc.danny.is
  → *.mc.danny.is A record → VPS
  → Nginx (on host, port 443) → matches server_name, basic auth
  → proxy_pass to 127.0.0.1:3100
  → mc-dashboard (Hono/Bun systemd service)
```

**Nginx** handles SSL termination and basic auth. The dashboard runs as a host systemd service (`mc-dashboard`) on port 3100 (localhost only via Nginx). Config is in `nginx/conf.d/dashboard.conf` (manually managed, not generated by `mc-generate`).
...
| 3100  | TCP      | Localhost  | Dashboard    | Hono/Bun, Nginx proxies to this             |
